...
class RightmoveData:
    ...
    def __init__(self, url: str):
        ...
        """
        self._url = url
        self._seen_links = dict()

    def parse(self, get_floorplans: bool = False):
        self._status_code, self._first_page = self._request(self._url)
        self._validate_url()
        self._results = self._get_results(get_floorplans=get_floorplans)
        return self._results

    def set_seen_links(self, links):
        """Set the links that have already been scraped.

        Args:
            links (list): list of links to properties that have already been
                scraped.
        """
        self._seen_links = links
    ...
    def _get_page(self, request_content: str, get_floorplans: bool = False):
        ...
        xp_descriptions = """//div[@class="propertyCard-description"]//a[@class="propertyCard-link"]//span/text()"""
        ...
        if get_floorplans:
            for i, weblink in enumerate(weblinks):
                ...
                if floorplan_url:
                    img_url = floorplan_url[0]
                    if "max" in img_url:
                        img_url = "_".join(img_url.split("_")[:-2]) + ".jpeg"
                    floorplan_urls.append(img_url)
                else:
                    floorplan_urls.append(np.nan)
